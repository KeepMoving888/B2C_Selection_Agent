...
1. **微调有效**：Merged 模型相对 Base 的 margin 提升 **+74.1%**，说明 ORPO 偏好对齐显著增强了模型对电商选品领域回答的区分能力。
2. **AWQ 保精度**：INT4 量化后 accuracy 仍保持 **100%**，avg margin 虽较 FP16 Merged 下降约 31%，但仍优于 Base 模型 **+19.7%**，满足生产部署的精度要求。
3. **显存与吞吐收益**：
   - FP16 Merged 约需 **14-16GB** 显存；
   - AWQ INT4 仅需 **6-8GB** 显存，单卡 4060Ti 即可部署；
